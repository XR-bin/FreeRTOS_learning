...
static void AppTaskCreate(void)
{
  ...
  if(NULL != MuxSemRecursive_Handle) printf("创建MuxSem_Handle互斥信号量成功!\r\n");
	xSemaphoreGive( MuxSemRecursive_Handle );//释放一次互斥信号量
...
static void LED_Task(void* parameter)
{
	while (1)
	{
		xSemaphoreTakeRecursive(MuxSemRecursive_Handle, //递归互斥信号量句柄
...
static void Key_Task(void* parameter)
{
	BaseType_t xReturn = pdTRUE; //定义一个获取互斥返回值，默认为pdTRUE
	static int num =0;
	...
	while (1)
	{
		switch(KEY_Scan())
		{
			case 1: 
				printf("获取互斥信号量\r\n");
				xReturn = xSemaphoreTakeRecursive(MuxSemRecursive_Handle, //递归互斥信号量句柄
																					portMAX_DELAY);         //等待时间(一直等)
				if(xReturn == pdTRUE) 
				{
					num++;
					printf("成功获取互斥信号量，num：%d\r\n",num);
				}
				else
				{
					printf("获取互斥信号量失败\r\n");
