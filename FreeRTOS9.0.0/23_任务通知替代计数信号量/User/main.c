  ...
  while(1)
	{
		if(KEY0_Scan() == 1)
		{
			...
			if(take_num > 0) printf("KEY0被按下，成功申请到停车位，当前车位为 %d\r\n", take_num);                      
			else             printf("KEY0被按下，车位已经没有了，请按KEY1释放车位\r\n");
		}
  ...
  while(1)
	{
		...
		if(KEY1_Scan() == 2)
		{
			...
			if( xReturn == pdPASS ) printf("KEY1被按下，释放1个停车位。\r\n");
		}
