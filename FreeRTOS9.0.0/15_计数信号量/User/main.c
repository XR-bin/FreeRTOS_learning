...
static void AppTaskCreate(void)
{
	...
	//创建计数信号量
  CountSem_Handle = xSemaphoreCreateCounting(5,5);	   //计数信号量最多有5个，初始计数信号量为5个
  if(NULL != CountSem_Handle) printf("CountSem_Handle计数信号量创建成功!\r\n");
	...
	if(xReturn == pdPASS) printf("Receive_Task任务创建成功!\r\n");
...
static void Take_Task(void* parameter)
{
	BaseType_t xReturn = pdTRUE;//定义一个计数信息返回值，默认为pdTRUE
	...
	while (1)
	{
		...
		if(KEY0_Scan() == 1)       
		{
			...
			xReturn = xSemaphoreTake(CountSem_Handle,	//计数信号量句柄
												       0); 	            //等待时间：0 
			if ( pdTRUE == xReturn ) printf( "KEY0被按下，成功申请到停车位。\r\n" );
			else                     printf( "KEY0被按下，不好意思，现在停车场已满！\r\n" );							
		}
...
static void Give_Task(void* parameter)
{
	...
	while (1)
	{
		...
		if(KEY1_Scan() == 2)       
		{
			...
			if ( pdTRUE == xReturn ) printf( "KEY1被按下，释放1个停车位。\r\n" );
			else                     printf( "KEY1被按下，但已无车位可以释放！\r\n" );							
		}
