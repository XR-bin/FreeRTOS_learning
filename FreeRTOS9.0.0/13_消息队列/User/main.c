...
static void AppTaskCreate(void)
{
  ...
  if(NULL != Test_Queue) printf("创建Test_Queue消息队列成功!\r\n");
	...
	if(xReturn == pdPASS) printf("Send_Task任务创建成功!\r\n");
...
static void Receive_Task(void* parameter)
{
	BaseType_t xReturn = pdTRUE;//定义一个消息队列信息返回值，默认为pdTRUE
  uint32_t r_queue;	          //定义一个接收消息的变量
    ...
    if(pdTRUE == xReturn) printf("本次接收到的数据是%d\r\n\r\n",r_queue);
    else                  printf("数据接收出错,错误代码0x%lx\n",xReturn);
	}
}
...
{
	BaseType_t xReturn = pdTRUE; //定义一个消息队列信息返回值，默认为pdTRUE
  uint32_t send_data1 = 1;     //数据1
  uint32_t send_data2 = 2;     //数据2
