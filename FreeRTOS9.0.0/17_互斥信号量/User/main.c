...
static void AppTaskCreate(void)
{
  ...
  if(NULL != MuxSem_Handle) printf("创建MuxSem_Handle互斥信号量成功!\r\n");
	xSemaphoreGive( MuxSem_Handle );//释放一次互斥信号量
	
	//创建LowPriority_Task任务
	xReturn = xTaskCreate(
								(TaskFunction_t	)LowPriority_Task,	       //任务函数
								(const char* 	  )"LowPriority_Task",       //任务名称
								(uint32_t 		  )128,					             //任务堆栈大小
								(void* 		  	  )NULL,				             //传递给任务函数的参数
								(UBaseType_t 	  )1, 				               //任务优先级
								(TaskHandle_t*  )&LowPriority_Task_Handle);//任务句柄
								
	if(xReturn == pdPASS) printf("LowPriority_Task任务创建成功!\r\n");
								
	//创建MidPriority_Task任务
	xReturn = xTaskCreate(
								(TaskFunction_t	)MidPriority_Task,		     //任务函数
								(const char* 	  )"MidPriority_Task",		   //任务名称
								(uint32_t 		  )128,					             //任务堆栈大小
								(void* 		  	  )NULL,				             //传递给任务函数的参数
								(UBaseType_t 	  )2, 				               //任务优先级
								(TaskHandle_t*  )&MidPriority_Task_Handle);//任务句柄 
																			
	if(xReturn == pdPASS) printf("MidPriority_Task任务创建成功!\r\n");
	
	//创建HighPriority_Task任务
	xReturn = xTaskCreate(
								(TaskFunction_t	)HighPriority_Task,		      //任务函数
								(const char* 	  )"HighPriority_Task",		    //任务名称
								(uint32_t 		  )128,					              //任务堆栈大小
								(void* 		  	  )NULL,				              //传递给任务函数的参数
								(UBaseType_t 	  )3, 				                //任务优先级
								(TaskHandle_t*  )&HighPriority_Task_Handle);//任务句柄 
																			
	if(xReturn == pdPASS) printf("HighPriority_Task任务创建成功!\r\n");
...
{
	static uint32_t i;
	BaseType_t xReturn = pdTRUE;//定义一个互斥信息返回值，默认为pdTRUE
